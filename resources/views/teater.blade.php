...
<html lang="en">
  ...
  <body>
        ...
        <div class="col-md-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    <h4>teater list
                        <a onclick="addForm()" class="btn btn-primary pull-right" style="margin-top: -8px;">Tambah teater</a>
                    </h4>
                </div>
                <div class="panel-body">
                    <table id="teater-table" class="table table-stripped">
                        <thead>
                            <tr>
                                <th>No </th>
                                <th>Nama Teater</th>
    ...
    <script type="text/javascript">
      var table = $('#teater-table').DataTable({
                      processing: true,
                      serverSide: true,
                      ajax: "{{ route('api.teater') }}",
                      columns: [
                        {data: 'DT_RowIndex', name: 'id'},
                        {data: 'nama_teater', name: 'nama_teater'},
      ...
      function editForm(id) {
        ...
        $('#modal-form form')[0].reset();
        $.ajax({
          url: "{{ url('teater') }}" + '/' + id + "/edit",
          type: "GET",
          dataType: "JSON",
          success: function(data) {
            $('#modal-form').modal('show');
            $('.modal-title').text('Edit teater');

            $('#id').val(data.id);
            $('#nama_teater').val(data.nama_teater);
          },
          error : function() {
              alert("Nothing Data");
          }
      ...
      $(function(){
            $('#modal-form form').validator().on('submit', function (e) {
                if (!e.isDefaultPrevented()){
                    var id = $('#id').val();
                    if (save_method == 'add') url = "{{ url('teater') }}";
                    else url = "{{ url('teater') . '/' }}" + id;
