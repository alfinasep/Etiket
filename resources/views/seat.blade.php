...
<html lang="en">
  ...
  <body>
        ...
        <div class="col-md-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    <h4>seat list
                        <a onclick="addForm()" class="btn btn-primary pull-right" style="margin-top: -8px;">Tambah Kursi</a>
                    </h4>
                </div>
                <div class="panel-body">
                    <table id="seat-table" class="table table-stripped">
                        <thead>
                            <tr>
                                <th>Id</th>
                                <th>Nama Seat</th>
    ...
    <script type="text/javascript">
      var table = $('#seat-table').DataTable({
                      processing: true,
                      serverSide: true,
                      ajax: "{{ route('api.seat') }}",
                      columns: [
                        {data: 'DT_RowIndex', name: 'id'},
                        {data: 'nama_seat', name: 'nama_seat'},
                        {data: 'tea', name: 'teater'},
      ...
      function editForm(id) {
        ...
        $('#modal-form form')[0].reset();
        $.ajax({
          url: "{{ url('seat') }}" + '/' + id + "/edit",
          type: "GET",
          dataType: "JSON",
          success: function(data) {
            $('#modal-form').modal('show');
            $('.modal-title').text('Edit seat');

            $('#id').val(data.id);
            $('#nama_seat').val(data.nama_seat);
      ...
      $(function(){
            $('#modal-form form').validator().on('submit', function (e) {
                if (!e.isDefaultPrevented()){
                    var id = $('#id').val();
                    if (save_method == 'add') url = "{{ url('seat') }}";
                    else url = "{{ url('seat') . '/' }}" + id;
